...
L.Draw.Polygon = L.Draw.Polyline.extend({
	...
	_getTooltipText: function () {
		var text;
		if (this._markers.length === 0) {
			text = 'Клик, чтобы начать рисование.';
		} else if (this._markers.length < 3) {
			text = 'Продолжайте, пока не нарисуете необходимую форму.';
		} else {
			text = 'Двойной клик замкнет форму.';
		}
		return {
			text: text
		};
...
L.Draw.SimpleShape = L.Draw.Feature.extend({
	...
	_onMouseMove: function (e) {
		...
		this._tooltip.updatePosition(latlng);
		if (this._isDrawing) {
			this._tooltip.updateContent({ text: 'Отпустите кнопку мыши, чтобы завершить.' });
...
L.EditToolbar = L.Toolbar.extend({
	...
	addToolbar: function (map) {
		...
		this._actionsContainer = this._createActions([
			...
			{
				title: 'Отмена editing, discards all changes.',
				text: 'Отмена',
				callback: this.disable,
				context: this
			}
